...
namespace StarterKit.DataAccess.Options
{
	...
	public class DataAccessSettingsMongo : SettingsBase
	{
		...
		public static void RegisterConfiguration(IServiceCollection services, IConfiguration configuration)
		{
			...
			services.Configure<DataAccessSettingsMongo>(GetConfigurationSection(configuration));
		}
	}

	public static class DataAccessExtensionsMongo
	{
		...
		private static Dictionary<string, string> GetEnvironmentVariablesDict(IHostEnvironment hostingEnv)
		{
			...
			{
				ConfigUtil.FillFromEnvironment(DataAccessSettingsConfigKeyMongo.Host, "DataAccess:ConnectionString:Host", environmentDict);
				ConfigUtil.FillFromEnvironment(DataAccessSettingsConfigKeyMongo.Port, "DataAccess:ConnectionString:Port", environmentDict);
				ConfigUtil.FillFromEnvironment(DataAccessSettingsConfigKeyMongo.DbName, "DataAccess:ConnectionString:DbName", environmentDict);
				ConfigUtil.FillFromEnvironment(DataAccessSettingsConfigKeyMongo.User, "DataAccess:ConnectionString:User", environmentDict);
				ConfigUtil.FillFromEnvironment(DataAccessSettingsConfigKeyMongo.PassWord, "DataAccess:ConnectionString:Password", environmentDict);
			}
