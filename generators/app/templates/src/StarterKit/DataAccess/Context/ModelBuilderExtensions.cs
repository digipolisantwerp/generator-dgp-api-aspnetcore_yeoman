    ...
    public static void LowerCaseTablesAndFields(this ModelBuilder modelBuilder)
    {
      foreach (var entityType in modelBuilder.Model.GetEntityTypes())
      {
        ...
        foreach (var property in entityType.GetProperties())
        {
          ...
          var columnNameAttribute = property.GetAnnotations()
            .FirstOrDefault(x => x.Name.ToLower().Contains("columnname"))?.Value;
          property.Relational().ColumnName = columnNameAttribute == null
            ? property.Name.ToLower()
            : columnNameAttribute.ToString().ToLower();
        }
      }
    }
