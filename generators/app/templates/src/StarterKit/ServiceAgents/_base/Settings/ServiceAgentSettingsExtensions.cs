	...
	public class ServiceAgentSettingsExtensions
	{
		public static void SetConfig(ServiceAgentSettings serviceAgentSettings, IHostEnvironment env)
		{
			...
			foreach (var agent in serviceAgentSettings.Services)
			{
				...
				settings.AuthScheme =
					ConfigUtil.GetEnvironmentVariable($"{agentConfigName}_AUTHSCHEME", true, settings.AuthScheme);

				settings.Host = ConfigUtil.GetEnvironmentVariable($"{agentConfigName}_HOST");
				settings.Path = ConfigUtil.GetEnvironmentVariable($"{agentConfigName}_PATH");
				settings.Port = ConfigUtil.GetEnvironmentVariable($"{agentConfigName}_PORT", true);
				settings.Scheme = ConfigUtil.GetEnvironmentVariable($"{agentConfigName}_SCHEME");

				settings.OAuthClientId =
					ConfigUtil.GetEnvironmentVariable($"{agentConfigName}_OAUTHCLIENTID", true, settings.OAuthClientId);
				settings.OAuthClientSecret = ConfigUtil.GetEnvironmentVariable($"{agentConfigName}_OAUTHCLIENTSECRET",
					true, settings.OAuthClientSecret);
				settings.OAuthPathAddition = ConfigUtil.GetEnvironmentVariable($"{agentConfigName}_OAUTHPATHADDITION",
					true, settings.OAuthPathAddition);
