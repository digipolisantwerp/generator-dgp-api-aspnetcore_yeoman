...
namespace StarterKit.ServiceAgents._base.Auth
{
  public class OAuthAgent : IOAuthAgent
  {
    ...
    public async Task<string> ReadOrRetrieveAccessToken<TServiceAgentSettings>(TServiceAgentSettings settings) where TServiceAgentSettings : AgentSettingsBase
    {
      var cacheKey = $"{settings.OAuthClientId}{settings.OAuthClientSecret}{settings.OAuthScope}{settings.OAuthTokenEndpoint}";

      var (succeeded, tokenReplyResult) = await _cache.GetFromCacheAsync<TokenReply>(cacheKey).ConfigureAwait(false);
