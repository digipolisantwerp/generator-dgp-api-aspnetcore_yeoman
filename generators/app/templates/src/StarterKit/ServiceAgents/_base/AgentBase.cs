...
namespace StarterKit.ServiceAgents._base
{
  public abstract class AgentBase<TAgent> : IDisposable where TAgent : class
  {
    ...
    protected AgentBase(ILogger<TAgent> logger, HttpClient httpClient, IServiceProvider serviceProvider)
    {
      ...
      if (settings.Value == null) throw new ArgumentNullException(nameof(ServiceAgentSettings), $"{nameof(ServiceAgentSettings)} cannot be null.");
      ...
      _jsonSerializerSettings = serviceProvider.GetRequiredService<IOptions<MvcNewtonsoftJsonOptions>>()?.Value?.SerializerSettings;
    }
