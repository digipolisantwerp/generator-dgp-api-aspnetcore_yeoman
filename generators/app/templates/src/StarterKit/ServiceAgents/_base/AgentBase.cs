...
{
    public abstract class AgentBase<TAgent> where TAgent : class
    {
        protected AgentBase(ILogger<TAgent> logger, HttpClient httpClient)
        {
        ...
        protected async Task<T> PostAsync<T>(string requestUri, T item)
        {
            ...
            {
                using (var response = await HttpClient.PostAsync(requestUri, contentPost))
        ...
        protected async Task<T> PutAsync<T>(string requestUri, T item)
        {
            using (HttpContent contentPost = new ByteArrayContent(Utf8Json.JsonSerializer.Serialize(item)))
            {
                using (var response = await HttpClient.PutAsync(requestUri, contentPost))
                {
                    return await ParseResult<T>(response);
                }
            }
        ...
        protected async Task PutWithEmptyResultAsync<T>(string requestUri, T item)
        {
            using (HttpContent contentPost = new ByteArrayContent(Utf8Json.JsonSerializer.Serialize(item)))
            {
                using (await HttpClient.PutAsync(requestUri, contentPost))
                { }
            }
        }
        ...
        {
            using (await HttpClient.DeleteAsync(requestUri))
            { }
        }
