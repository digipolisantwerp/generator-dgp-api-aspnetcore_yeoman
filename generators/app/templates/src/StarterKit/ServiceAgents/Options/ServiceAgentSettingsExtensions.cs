  ...
  public class ServiceAgentSettingsExtensions
  {
    public static void SetConfig(ServiceAgentSettings serviceAgentSettings, IHostEnvironment env)
    {
      ...
      foreach (var agent in serviceAgentSettings.Services)
      {
        ...
        settings.BasicAuthPassword = ConfigUtil.GetEnvironmentVariable($"{agentConfigName}_BASICAUTHPASSWORD", true, settings.BasicAuthPassword);
        settings.BasicAuthUserName = ConfigUtil.GetEnvironmentVariable($"{agentConfigName}_BASICAUTHUSERNAME", true, settings.BasicAuthUserName);
        ...
        settings.Port = ConfigUtil.GetEnvironmentVariable($"{agentConfigName}_PORT", false);
