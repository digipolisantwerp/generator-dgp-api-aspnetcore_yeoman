...
namespace StarterKit.UnitTests.DataAccess.Options.ConnectionStringTests
{
  public class InstantiationTests
  {
    ...
    {
      var ex = Assert.Throws<ArgumentNullException>(() => new ConnectionString(ConnectionType.PostgreSQL, null, 1234, "db", "user", "pwd"));
    ...
    {
      var ex = Assert.Throws<ArgumentException>(() => new ConnectionString(ConnectionType.PostgreSQL, "", 1234, "db", "user", "pwd"));
      Assert.Equal("host", ex.ParamName);
    }
    ...
    {
      var ex = Assert.Throws<ArgumentNullException>(() => new ConnectionString(ConnectionType.PostgreSQL, "host", 1234, null, "user", "pwd"));
    ...
    {
      var ex = Assert.Throws<ArgumentException>(() => new ConnectionString(ConnectionType.PostgreSQL, "host", 1234, "", "user", "pwd"));
      Assert.Equal("dbname", ex.ParamName);
    }
